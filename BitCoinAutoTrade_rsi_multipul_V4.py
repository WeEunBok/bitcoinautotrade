...
#            file.flush()
        ...
        if rsi_gubun == -1:
            if (rsi_arr[29] > 35) or ((rsi_arr[27] - rsi_arr[28]) <= 0 and (rsi_arr[28] - rsi_arr[29]) > 0):
                rsi_gubun = 1
        elif rsi_gubun == 1:
            if (rsi_arr[27] - rsi_arr[28]) > 0 and (rsi_arr[28] - rsi_arr[29]) < 0:
                #krw = current_krw / 100
                ...
                if current_krw >= krw:
                    ...
                    #data = "BUY_COIN2!  : %f \n" % current_price
                    data = "now_date : %s --- BUY_COIN2! : %s  rsi : %s / %s / %s\n" % (datetime.datetime.now(), current_price, rsi_arr[27], rsi_arr[28], rsi_arr[29])
                    file.write(data)
                    file.flush()
                    #data = "now_date : %s \n" % datetime.datetime.now()
                    #file.write(data)
...
#                rsi_gubun = 1
#        elif rsi_gubun == 1:
#            if (rsi_arr[27] <= 35 or rsi_arr[28] <= 35 or rsi_arr[29] <= 35) and (rsi_arr[27] - rsi_arr[28]) > 0 and (rsi_arr[28] - rsi_arr[29]) < 0:
#                #krw = current_krw / 100
#                if current_price > avg_price:
#                    krw = 9000
#                elif current_price == avg_price:
...
#                    rsi_gubun = -1
        ...
        if krw == 0:
            #rsi 35미만(과매도시) 매수
            if rsi_arr[29] < 35 and (rsi_arr[27] - rsi_arr[28]) > 0 and (rsi_arr[28] - rsi_arr[29]) < 0:
                #krw = current_krw / 100
                krw = 10000
                buy_money = current_price
                #if krw > 5000:
                if current_krw >= krw:
                    #upbit.buy_market_order(KRW_coin, krw*0.9995) # 비트코인 매수
                    #data = "BUY_COIN!  : %f \n" % current_price
                    data = "now_date : %s --- BUY_COIN!! : %s  rsi : %s / %s / %s\n" % (datetime.datetime.now(), current_price, rsi_arr[27], rsi_arr[28], rsi_arr[29])
                    file.write(data)
                    file.flush()
                    #data = "now_date : %s \n" % datetime.datetime.now()
                    #file.write(data)
        ...
        if krw != 0:
            #rsi 60이상(과매수시) 매도
            if rsi_arr[29] >= 60 and (rsi_arr[27] - rsi_arr[28]) <= 0 and (rsi_arr[28] - rsi_arr[29]) > 0:
                coin_price = get_balance(coin)
                #upbit.sell_market_order(KRW_coin, coin_price) # 비트코인 전량 매도
                #data = "SELL_COIN! : %f \n" % current_price
                data = "now_date : %s --- SELL_COIN! : %s  rsi : %s / %s / %s\n" % (datetime.datetime.now(), current_price, rsi_arr[27], rsi_arr[28], rsi_arr[29])
                file.write(data)
                file.flush()
                #data = "now_date : %s \n" % datetime.datetime.now()
                #file.write(data)
