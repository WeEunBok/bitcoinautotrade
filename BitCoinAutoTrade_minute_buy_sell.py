...
###############################################################
#최근 체결가
#file.write(pyupbit.get_current_price("KRW-BTC"))

#현재가
#file.write(pyupbit.get_current_price(["KRW-BTC", "KRW-XRP"]))
...
#file.write(pyupbit.get_ohlcv("KRW-BTC", interval="day")              # 일봉 데이터 (5일)
#file.write(pyupbit.get_ohlcv("KRW-BTC", interval="minute1"))         # 분봉 데이터
#file.write(pyupbit.get_ohlcv("KRW-BTC", interval="minute5"))         # 분봉 데이터
#file.write(pyupbit.get_ohlcv("KRW-BTC", interval="week"))            # 주봉 데이터
...
#file.write(pyupbit.get_orderbook(tickers=["KRW-BTC", "KRW-XRP"]))
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            #1분봉 3개 가지고오기
            minute_price = pyupbit.get_ohlcv("KRW-BTC", interval="minute1", count=3)
...
#                        upbit.buy_market_order("KRW-BTC", krw*0.9995) # 비트코인 매수
            ...
            if krw != 0:
                if ((current_price / krw) - 1) * 100 > 0.5:
                    data = "익절이다! : %f \n" % current_price
                    file.write(data)
                    krw = 0
            ...
            if krw != 0:
                if ((current_price / krw) - 1) * 100 < -1.0:
                    data = "손절이다! : %f \n" % current_price
                    file.write()
                    krw = 0
                    #upbit.sell_market_order("KRW-BTC", BTC*0.9995) # 비트코인 전량 매도
